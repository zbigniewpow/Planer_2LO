...
// columns: [{ id, label, verticalLabel? }]
...
export default function PrintMasterGrid({ columns, getCellLessons, verticalHeaders = false }) {
  return (
    <table className="w-full table-fixed border-collapse text-[7px] leading-[8px]">
      <colgroup>
      ...
      <thead>
        <tr>
          ...
          {columns.map((c) => (
            <th
              key={c.id}
              className={`border border-slate-300 bg-slate-100 px-0.5 py-0.5 font-semibold text-slate-700 ${
                verticalHeaders ? 'h-[70px] whitespace-nowrap [writing-mode:vertical-rl]' : ''
              }`}
            >
              {c.label}
            </th>
        ...
        {DAYS.map((day) =>
          HOURS.map((h, hIdx) => (
            <tr key={`${day.value}-${h.hour}`}>
              ...
              <td className="border border-slate-300 px-0.5 text-center text-slate-500">
                <div className="font-semibold text-slate-700">{h.hour + 1}</div>
                <div className="text-[6px] leading-[7px]">{h.start}</div>
              </td>
              {columns.map((col) => {
                ...
                const text = cellLessons
                  ...
                  .join('/')
                return (
                  <td key={col.id} className="overflow-hidden truncate border border-slate-200 px-0.5">
                    {text}
                  </td>
                )
              })}
